...
def get_model(flags):
    ...
    config.separate_qkv = flags.separate_qkv  # perrysc
...
def train_llama(flags):
            ...
            if training_ustep % flags.grad_accum_usteps == 0:
                ...
                def _print_logs(running_loss_reduced_detached, total_norm):
                    if is_root and not extract_graphs_only:
                        ...
                        # percent_epoch_complete = (training_ustep / len(train_loader) * 100)  # perrysc
                        percent_epoch_complete = (global_step * flags.grad_accum_usteps / len(train_loader) * 100)
                        logger.log(
                            epoch,
                            global_step,
    ...
    # load and resume (fine-tune) from a converted HF pretrained checkpoint
    if flags.load_pretrained_checkpoint:
        xm.master_print("Loading pretrained checkpoint for fine-tuning.")
        checkpointing.load(args.pretrained_checkpoint_dir, model_or_optimizer=model, model_key=None)
        epoch = 0
        global_step = 0

    # resume from a previous NxD pretraining checkpoint
...
if __name__ == "__main__":
    ...
    group.add_argument(
        ...
        default=False,
        action="store_true",
        help="load a pretrained checkpoint to begin fine-tuning. expects a converted HF pretrained checkpoint in --pretrained_checkpoint_dir"
    )
    parser.add_argument(
        "--pretrained_checkpoint_dir",
        type=str,
        default=None,
        help="Path to the pretrained checkpoint dir used to start fine-tuning"
    )
    group.add_argument(
        "--resume_ckpt",
        action="store_true",
